...
from agents.buffer_tsf_phi import ReplayBuffer_TSF_PHI
from tasks.reacher_tsf_phi import Reacher_TSF_PHI
...
config_params = parse_config_file('reacher_tsf_phi.cfg')
...
def generate_tasks(include_target):
    ...
    train_tasks = [Reacher_TSF_PHI(all_goals, i, feature_dim, include_target) for i in range(len(goals))]
    test_tasks = [Reacher_TSF_PHI(all_goals, i + len(goals), feature_dim, include_target) for i in range(len(test_goals))]
